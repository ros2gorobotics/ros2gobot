...
The hardware interface component for this robot is implemented by [`ros2gobot_base`](../ros2gobot_base/) package. It is in charge of providing the necessary state and command interfaces to be claimed by the controllers.
...
The hardware interface to be activated when `controller_manager` node is activated is indicated via robot description passed via ROS 2 Parameter. A tag `<ros2_control>` is expected to be present in the robot description with the information of the hardware components to be activated as well as the state and command interfaces. See [`ros2gobot_control.urdf.xacro`](../ros2gobot_description/urdf/include/ros2gobot_control.urdf.xacro).
...
Each controller accepts ROS 2 parameters which they are provided via [`ros2gobot_controllers.yaml`](config/ros2gobot_controllers.yaml). 
